...
| Item | Where | Done |
|---|---|---|
| Competition repo (public) + released tag | https://github.com/macrocosm-os/apex-competition-humanoid-parkour-v2 @ `v0.2.0` | ☑ |
| `spec.yaml` (`apex.competition.v1`) — `apex-dev preflight` passes | `spec.yaml` (repo root; preflight ✓ 2026-07-31 against toolkit `d063d90`) | ☑ |
| Player image | `ghcr.io/macrocosm-os/apex-competition-humanoid-parkour-v2-player@<digest>` (cosign-signed on tag v0.2.0 — digest pinned in `spec.yaml` after the release run) | ☑ |
| Referee image | `ghcr.io/macrocosm-os/apex-competition-humanoid-parkour-v2-referee@<digest>` (cosign-signed on tag v0.2.0 — digest pinned in `spec.yaml` after the release run) | ☑ |
