...
# Kept deliberately compact: every entry needs a policy with only the G1's
# twelve leg actuators to be a plausible competitor.
...
# Per-event limits make a literal 400 m feasible without granting every short
# event the same long wall-clock allowance. They are included in task history.
EVENT_MAX_STEPS = {
    "sprint_100": 2500,
    "sprint_400": 5000,
    "hurdles_100": 3000,
...
def _hurdles_100() -> EventLayout:
    ...
    # Ten 0.55 m barriers, with a long enough run-in to settle into a sprint.
    for x in np.linspace(12.0, 88.5, 10):
        surfaces.append(Surface("hurdle", float(x), 0.0, PLINTH_TOP + 0.275,
                                0.12, TRACK_HALF_W, 0.275, walkable=False))
...
def _high_jump(challenge: Mapping[str, float]) -> EventLayout:
    bar_height = float(challenge.get("bar_height_m", 0.95))
    bar_x = 12.0
    ...
    surfaces.append(Surface("bar", bar_x, 0.0, PLINTH_TOP + bar_height,
                            0.045, TRACK_HALF_W, 0.035, walkable=False))
...
def _triple_jump() -> EventLayout:
    # The hop, step, and jump are three real gaps. A valid long final landing
    # only follows from negotiating all three phases rather than one long run.
    takeoff, landing = 12.0, 23.0
    surfaces = [
        _slab(-3.0, takeoff + 3.0, kind="track"),
        _slab(13.5, 2.0, kind="landing"),
        _slab(17.0, 1.8, kind="landing"),
        _slab(landing, 24.0, kind="sand"),
    ]
